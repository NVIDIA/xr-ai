...
| Agent functions | `agent-sdk/xr-ai-nat/` | Typed NAT functions plus participant-scoped event delivery between them |
...
as application-labeled UI text without invoking speech synthesis. Its session,
baseline, captions, and importance decisions are also persisted as JSON Lines.
...
`http://<host>:8092` shows transcript, video-log, and structured agent activity
alongside visual change-watcher decisions for demo recording.
