...
def _content_tool_call(content: str, offered: dict[str, ToolDef]) -> dict[str, Any] | None:
    """Recover a tool call the serving layer failed to parse out of content.

    Some models occasionally emit the call as bare JSON in the message body;
    the server's tool-call parser then misses it and downstream agents treat
    the turn as a final answer. Only a lone JSON object is recovered: one
    naming an offered tool, or one of two or more keys that fit exactly one
    offered tool's schema."""
    text = content.strip()
    ...
    if not isinstance(data, dict):
        return None
    for wrapper_key in ("command", "function", "tool_call"):
    ...
    if isinstance(name, str) and name not in offered:
        # Models sometimes drop the group prefix from the tool name.
        matches = [candidate for candidate in offered if candidate.endswith(f"__{name}")]
    ...
    if isinstance(name, str) and name in offered:
        ...
        if arguments is None:
            arguments = {
                key: value for key, value in data.items() if key not in ("name", "tool", "action", "id", "type")
            }
        ...
        if not isinstance(arguments, dict):
            return None
        return {"name": name, "args": arguments, "id": _recovery_id(), "type": "tool_call"}
    # A single-key object is as likely a data echo as a call (a bare
    # argument echo would match any one-required-field tool, executing a
    # narration as a mutation), so shape inference needs two or more keys.
    if name is not None or len(data) < 2:
        return None
    candidates = []
    for tool in offered.values():
        properties = tool.parameters.get("properties", {})
        required = set(tool.parameters.get("required", ()))
        if set(data) <= set(properties) and required <= set(data):
...
def _recovery_id() -> str:
    return f"content-recovered-{next(_RECOVERY_COUNTER)}"
        ...
        **kwargs: Any,
        ...
        if self.recover_tool_calls and not tool_calls and raw_tools:
            ...
            if (recovered := _content_tool_call(_text(content), offered)) is not None:
                # The original text moves to additional_kwargs: left in
                # content, the model imitates its own leaked JSON on later
                # turns, and it becomes the final answer at the iteration
                # cap; dropped entirely, a false-positive recovery is
                # undiagnosable.
