...
- **video-memory-service** owns recorded chunk queries, NVDEC, and PNG output
  ...
  `video_recording.out_dir`. Historical sampling accepts a duration and a
  hard total frame budget, decoding each selected chunk once. Optional maximum
  width and height bounds preserve aspect ratio and never upscale the exported
  PNGs. Current frames stay with the caller's hub client.
