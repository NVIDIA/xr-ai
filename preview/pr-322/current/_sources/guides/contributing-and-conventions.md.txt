...
|---|---|---|
| Python | `server-runtime/`, `agent-sdk/`, `utils/`, `services/`, `agent-mcp-servers/`, `agent-samples/`, `tests/` | Target Python 3.11+ (CI runs 3.11 and 3.12); follow PEP 8; use type annotations and f-strings; manage environments with `uv` (each sub-project is its own uv project — run `uv sync` in its directory). |
