...
For Piper this sends `SIGTERM` to its verified launcher-owned process group,
waits up to 20 s, then sends `SIGKILL`. Piper remains in that group so the same
escalation also reaches it during an aborted stack startup. Cleanup falls back
to the listener PID if process-group ownership cannot be verified, and other
local servers always use PID-only cleanup. For docker-mode servers it runs
...
success if server ownership cannot be verified or any target remains running.
