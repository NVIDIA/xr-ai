...
an ended delivery scope. Pass `context=nemo_relay.fork_asyncio_context()` to
`asyncio.create_task()`, then enter
`nemo_relay.use_scope_stack(nemo_relay.create_scope_stack())` inside the
detached coroutine before opening its operation scope or calling tools.
