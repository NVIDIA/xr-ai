...
- **Vision keeps acquisition and inference composable.** The native `xr_vision`
  group accepts an acquired image path and owns normalization plus VLM
  invocation; VLM MCP only republishes that function. The higher-level
  `xr_vision_tools` group (`look_at_current_frame` / `look_at_past_frame`)
  acquires a participant's live or recorded frame itself and calls a
  `VLMService` from `xr-ai-models`; recorded lookups resolve through the
  `xr_video_memory` group.
...
network boundary. `VisionFunctionsConfig` (`xr_vision`) provides composable
path-based image question answering, while `VisionToolsConfig`
(`xr_vision_tools`) adds `look_at_current_frame` / `look_at_past_frame` over an
injected `xr-ai-models` VLM and acquires the participant's live or recorded
frame itself. `XRTrackingFunctionsConfig` exposes the current user frame through
