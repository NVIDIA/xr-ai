...
"""Pin the content-leak tool-call recovery in the shared LangChain adapter.
...
def test_lone_argument_echo_never_recovers():
    # A one-key echo would uniquely match remove_object's schema and delete
    # an object; shape inference requires at least two keys.
    assert _content_tool_call('{"object_words": "the blue sphere"}', OFFERED) is None


def test_two_key_shape_inference_recovers_unique_tool():
    recovered = _content_tool_call('{"object_words": "the cone", "factor": 0.5}', OFFERED)
    assert recovered and recovered["name"] == "object_ops__resize_object"


def test_ambiguous_shape_inference_rejected():
    two = dict(OFFERED)
    two["other_ops__grow"] = _tool("other_ops__grow", ["object_words", "factor"])
    assert _content_tool_call('{"object_words": "x", "factor": 2}', two) is None
