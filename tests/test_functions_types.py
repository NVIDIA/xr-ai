...
import pytest
from xr_ai_nat.functions import types
...
def test_value_models_stringify_as_json() -> None:
    ...
    frame = types.SpatialFrame(
        origin=types.Vector3(x=0.0, y=0.0, z=0.0),
        forward=types.Vector3(x=0.0, y=0.0, z=-1.0),
        right=types.Vector3(x=1.0, y=0.0, z=0.0),
        up=types.Vector3(x=0.0, y=1.0, z=0.0),
    )
...
def test_service_result_retains_unknown_fields() -> None:
    ...
    vec = types.Vector3.model_validate({"x": 1.0, "y": 2.0, "z": 3.0, "w": 9.0})
    assert vec.model_dump()["w"] == 9.0
    assert json.loads(str(vec))["w"] == 9.0
    assert isinstance(vec, types.ServiceResult)
...
def test_service_result_is_exported() -> None:
    ...
    assert "ServiceResult" in types.__all__
...
def test_spatial_math_schemas_is_a_deprecated_alias() -> None:
    ...
    assert legacy.Vector3 is types.Vector3
    assert legacy.SpatialFrame is types.SpatialFrame
