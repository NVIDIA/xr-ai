...
"""Unit tests for CloudXR runtime process launch behavior."""
...
def _install_cloudxr_runtime_fakes(monkeypatch):
    ...
    cloudxr_package = types.ModuleType("isaacteleop.cloudxr")
    ...
    env_config.get_env_config = lambda: types.SimpleNamespace(
        openxr_run_dir=lambda: "/tmp"
    )
    ...
    runtime.terminate_or_kill_runtime = lambda _process: None
    ...
    monkeypatch.setitem(sys.modules, "isaacteleop.cloudxr", cloudxr_package)
...
def test_start_runtime_process_uses_spawn_context(monkeypatch):
    ...
    class FakeContext:
        def Process(self, target):  # noqa: N802
