...
such as `hatchling` are not part of a uv lock. Both files are generated by
`uv run --script .github/scripts/generate_dependency_manifest.py`, which the
pre-commit hook runs on any `pyproject.toml` change and `lock-check` verifies
with `--check`. Nothing installs from that directory. A cutoff advance
regenerates the lock the same way after `uv.toml` changes; two in-flight
dependency changes that both touch the lock are reconciled by re-running the
script, not by merging the file by hand.
...
| Any `pyproject.toml` dependency or project metadata | Regenerate this map, the dependency manifest and its lock (pre-commit does both), and the affected project's gitignored `uv.lock` |
