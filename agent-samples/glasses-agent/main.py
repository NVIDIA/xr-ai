...
"""
glasses-agent orchestrator — always-on AI assistant for smart glasses.
...
Process stack:
  ...
  worker          → glasses_agent_worker  (this sample's worker)

How to run (from agent-samples/glasses-agent/):
    uv sync && uv run glasses_agent

To stop persisted model servers without restarting the full stack:
    uv run glasses_agent --stop
"""
import argparse
import datetime
import os
...
PROCESSES = [
    ...
    Process("worker",         "worker",                                  "glasses_agent_worker",
            config="yaml/glasses_agent_worker.yaml"),
]
...
_DEFAULT_TRACE_LOG   = Path("/tmp/glasses-agent-trace.log")
...
def run() -> None:
    ...
    p.add_argument("--run-dir", type=Path, default=None, metavar="DIR",
                   help="Directory for transcripts, frames, and trace log. "
                        "Default: auto-generated under /tmp/xr-glasses/.")
    ...
    run_dir: Path = ns.run_dir or (
        Path("/tmp/xr-glasses") / datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    )
    run_dir.mkdir(parents=True, exist_ok=True)
    os.environ["XR_RUN_DIR"] = str(run_dir)

    setup_logging("orchestrator", namespace="glasses-agent")
