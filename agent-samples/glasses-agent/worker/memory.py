...
class Demonstration:
    ...
    summary:      str = ""   # condensed description generated by LLM after recording
...
class AgentMemory:
    ...
    def add_observation(self, obs: Observation) -> None:
        ...
        # If currently recording, add as a demo step too.
    ...
    def start_recording(self, name: str) -> None:
        """Begin a new demonstration recording.  Discards any prior in-progress
        recording for the same name so the user can re-record cleanly."""
    ...
    def record_step(self, obs: Observation) -> None:
        """Append *obs* as the next step in the current recording.

        Called both by add_observation (background loop) and directly from
        QueryProcessor (explicit step capture).  No-op if not recording.
        """
        if self._recording is None:
            return
        step = DemoStep(
            step_number  = len(self._recording.steps) + 1,
            timestamp_us = obs.timestamp_us,
            description  = obs.description,
            image_path   = obs.image_path,
        )
        self._recording.steps.append(step)
        log.debug("demo step %d recorded", step.step_number)

    def finish_recording(self) -> Demonstration | None:
        """End the current recording, store it, and return it.

        Returns ``None`` if no recording was in progress (idempotent).
        """
        if self._recording is None:
        ...
        if demo.steps:
            ...
            log.info("demo recorded  name=%r  steps=%d", demo.name, len(demo.steps))
        else:
            log.warning("demo %r had no steps — discarding", demo.name)
        return demo
