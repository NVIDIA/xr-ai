...
| `worker/glasses_nat_tasks.py` | New `derive_step_key_info_impl`; key-info-guided + background-tolerant prompts in the two-tier completion check; `_key_info_correction`. **NAT-native LLM:** replaced hand-rolled `_post_chat` httpx with a `_chat(llm, …)` helper over NAT's LangChain LLM. **Wrong-object fix:** tier-1/tier-2 prompts now enforce object *identity* (a different object is a mismatch, not a match); the parser blocks completion on any `visible=false` check (an unmet requirement) instead of passing on any single visible check; new tier-3 `_diagnose_mistake` actively names the mistake so the student always hears a correction. |
...
| `worker/agent.py` | **Voice I/O migrated onto the unified pipecat pipeline** (like simple-vlm-example / xr-render-demo). The hand-rolled `GlassesAgent` (own `ProcessorEndpoint` + VAD + STT/TTS) is split into: `GlassesBrain` — a `xr_ai_pipecat.BrainProcessor` that runs the noise/intent gate (in `process_frame`, before the turn spawns) and delegates each accepted turn to `QueryProcessor` (NAT stays in the brain), mapping its `say`/`send_text`/`flush_audio` onto pipeline frames (`TextFrame`+`BrainResponseEndFrame` / `send_return_data` / `InterruptionFrame`); and `GlassesPerception` — the background VLM-observation + recording-capture + condenser loops (NAT-MCP, off the audio path), keeping `recording_warmup_s`. |
| `worker/vad.py` | **Removed** — VAD/STT/TTS now come from the pipecat pipeline (`xr-ai-pipecat`, which pulls `xr-ai-vad`'s Silero detector). |
| `worker/glasses_agent_nat_worker.py` | Compose the pipeline via `xr_ai_pipecat.make_voice_pipeline` (always-on `VoiceGateConfig`, `text_topic=""` since `QueryProcessor` sends its own data echoes); build `make_stt`/`make_tts`; wire `GlassesBrain` + `GlassesPerception`; run with `PipelineRunner`; close everything on shutdown. |
| `worker/pyproject.toml` | Depend on `xr-ai-pipecat` + `xr-ai-voicegate` + `pipecat-ai` + `xr-ai-models` (VAD via `xr-ai-pipecat → xr-ai-vad`); drop direct `numpy`/`Pillow`/`xr-ai-vad`; keep the `torchaudio` minor pin so Silero loads. |
