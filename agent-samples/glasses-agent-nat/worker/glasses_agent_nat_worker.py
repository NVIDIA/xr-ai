...
async def main(cfg: WorkerConfig, ready_file: pathlib.Path | None = None) -> None:
    ...
    try:
        ...
        # ── main agent ────────────────────────────────────────────────────────
        # STT / TTS endpoints come from cfg.models_yaml via xr-ai-models inside
        # GlassesAgent. The cfg.stt_server / cfg.tts_server fields are kept for
        # the readiness probes above (non-AI-service HTTP).
        agent = GlassesAgent(
            cfg               = cfg,
            memory            = memory,
