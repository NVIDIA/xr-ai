...
def normalize_nat_result(value: Any) -> dict[str, Any] | list[Any] | str | None:
    """Normalize NAT / FastMCP outputs to plain dict / list / str / None.

    Handles the shape variation across NAT function-group return types:
    raw primitives, MCP ``CallToolResult`` with ``structured_content`` or
    ``content[0].text``, pydantic models with ``model_dump``, or a JSON
    string. Falls through to ``str(value)`` for anything unrecognized.

    Re-used by both ``NatRuntime`` itself (worker-side calls) and by
    task implementations that invoke MCP-client functions directly.
    """
...
class NatRuntime:
    ...
    async def run_agent(self, prompt: str, *, participant_id: str = "") -> str:
        ...
        text = normalize_nat_result(result)
    ...
    async def call_tool(
        ...
        *,
        ...
        return normalize_nat_result(result)
