...
from nat_runtime import normalize_nat_result
...
def _payload(value: Any) -> dict[str, Any] | list[Any] | str | None:
    """Back-compat alias around the shared NAT result normalizer."""
    return normalize_nat_result(value)
...
async def condense_observations_impl(
    ...
    *,
    ...
    raw = (resp.content or "").strip()
    # Reuse the same JSON-from-text extractor used by analyze_recording so
    # markdown fences, prose prefixes, and trailing commentary all collapse
    # to the same robust path. Previous `raw.split("\`\`\`")[1].lstrip("json")`
    # broke on `\`\`\`JSON`, no-language fences, or prose-wrapped output.
