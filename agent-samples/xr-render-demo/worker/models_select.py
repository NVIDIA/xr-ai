...
"""Per-role model backend selection for xr-render-demo.

``model_backend`` in the worker YAML routes each model role (stt, tts, llm,
vlm) to a models file: ``local`` reads the file named by ``models_yaml``
(default models.yaml), ``nim`` reads models.nim.yaml, ``nim_local`` reads
...
optional ``default`` key for unlisted roles (``local`` when absent). Role
``llm`` covers both the ``llm`` and ``agent_llm`` model entries. Kept in
sync with the same module in simple-vlm-example/worker (deliberate
...
_ROLES = ("stt", "tts", "llm", "vlm")
# Model entries each role selects from its backend's models file. The llm
# role owns both LLM entries — one backend serves the reactive and agentic
# models.
_ROLE_ENTRIES: dict[str, tuple[str, ...]] = {
    "stt": ("stt",),
    "tts": ("tts",),
    "llm": ("llm", "agent_llm"),
    "vlm": ("vlm",),
