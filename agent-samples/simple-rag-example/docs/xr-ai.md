...
### RAG agent (procedure-compliance assistant with live vision)

Speak into the mic, type into the data channel, or send `"ping"` — all routes
trigger a two-step pipeline: (1) the rag-mcp server retrieves the most relevant
passages from a folder of `.txt` and `.md` files, (2) those passages plus the
latest camera frame are forwarded together to the VLM, which compares what it
sees against the documented procedures and reports compliance or missing steps.
Replies arrive as streaming Piper TTS audio and a `rag.response` text message.

Drop any device manuals, assembly procedures, safety notices, or SOPs into
`agent-samples/rag-agent/docs/` and restart; no code changes needed.

Uses `nvidia/Cosmos-Reason1-7B` for vision + reasoning (same model as
`simple-vlm-example`).

```bash
cd agent-samples/rag-agent
uv sync && uv run rag_agent
```

When ready, open `https://localhost:8080`, connect, and try:

- Type `ping` → the agent surveys the camera view and checks for procedure compliance.
- Type a question → retrieved document context + visual observation + VLM answer.
- Speak into your mic → transcribed and answered the same way.
- Point the camera at an assembly step → ask "am I doing this right?" for compliance feedback.

**Adding documents** — copy `.txt` or `.md` files into
`agent-samples/rag-agent/docs/` and restart the agent.  The rag-mcp server
indexes the corpus at startup (no separate indexing step).

**Pointing at a different corpus** — set `docs_dir` in
`agent-samples/rag-agent/yaml/rag_mcp_server.yaml` to any directory.

**Reusing the RAG server** — any other agent can connect to rag-mcp-server
at `http://localhost:8240/mcp` and call the `retrieve` tool directly.

| Resource | VRAM |
|---|---|
| rag-agent (Cosmos-7B VLM + STT + Piper TTS + rag-mcp) | ~23 GB |
