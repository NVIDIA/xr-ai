...
"""
glasses-agent-langchain worker — always-on AI assistant for smart glasses.
...
Launch:
  cd glasses-agent-langchain/worker && uv run glasses_agent_langchain_worker
"""
...
log = logging.getLogger("glasses_agent_langchain")
...
def _setup_trace_log(path: str = "/tmp/glasses-agent-langchain-trace.log") -> None:
    ...
    _trace_log.info("=== glasses-agent-langchain trace started ===")
...
async def main(cfg: WorkerConfig, ready_file: pathlib.Path | None = None) -> None:
    ...
    _trace_path = (
        str(pathlib.Path(_run_dir) / "trace.log")
        if _run_dir else "/tmp/glasses-agent-langchain-trace.log"
    )
    _setup_trace_log(_trace_path)

    log.info("glasses-agent-langchain starting")
    ...
    # ── connect to MCP servers + load LangChain tools ─────────────────────────
    async with (
        ...
        log.info("glasses-agent-langchain running")
        try:
            await agent.run()
        finally:
            agent.shutdown()
    ...
    log.info("glasses-agent-langchain stopped")
...
def run() -> None:
    ...
    setup_logging("worker", namespace="glasses-agent-langchain")
