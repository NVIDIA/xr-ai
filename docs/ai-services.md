...
[`services/`](../services/README.md); pick an LLM per sample based on the
...
For `simple-vlm-example`, set
`models_config: models.hosted-nim.json`; its stdlib-only orchestrator reads
ownership and credential metadata from that JSON profile. The render demo
retains its existing `model_backend` and flat-YAML selection until its owning
refactor migrates it.
...
- **llama-nemotron-llm** is a thin wrapper around `vllm serve` for
  ...
  [`services/llama-nemotron-llm/README.md`](../services/llama-nemotron-llm/README.md)
...
- **nemotron3-nano-llm** is a thin wrapper around `vllm serve` for
  ...
  [`services/nemotron3-nano-llm/README.md`](../services/nemotron3-nano-llm/README.md)
