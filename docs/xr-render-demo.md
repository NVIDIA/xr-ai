...
sample that follows the same multi-LLM + native-function pattern. For the user-facing
...
6. **Turn appended** to a rolling 4-turn history buffer — injected as
   ...
   to native text memory; model scratch output and tool traces are not stored.
...
(`XR_MND_HEADLESS`). `video-memory-service` owns live hub IPC and
recorded-video decoding. NAT Functions provide the typed in-process tool
surface over those services. Generic MCP adapters can republish selected
Functions for non-NAT consumers, but the demo does not launch or call them.
