...
file transfer, and network metrics remain separate operations.
...
The default HTTPS server leaf is signed by a development root CA stored under
`~/.local/share/xr-ai/`. Browsers allow a development click-through; Android
and Apple devices install the public root from `/cert` using the platform trust
procedures below. Use a public or managed certificate for production.
...
matching versioned tarball when present, otherwise reuses the matching
IsaacTeleop tarball at `~/hub/IsaacTeleop/deps/cloudxr/` when available, and
finally downloads the public NGC tarball. It installs npm dependencies, writes
both ESM bundles to `../web-xr/vendor/`, and records the SDK version beside
them. The orchestrator automatically rebuilds missing or stale bundles.
To bump CloudXR, edit `.sdk-version` and re-run the script. To bump LiveKit,
edit `package.json`, remove `node_modules`, and re-run the script.
...
The C++20 sample requires LiveKit C++ SDK v1.10.2 or newer.
