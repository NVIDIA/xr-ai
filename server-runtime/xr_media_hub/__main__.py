...
def _resolve_http_log_level(cfg) -> str:
    ...
    field; file capture is unaffected (DEBUG always). Accepts either a
    dict or a dataclass-style object."""
    if isinstance(cfg, dict):
        val = cfg.get("http_log_level")
    else:
        val = getattr(cfg, "http_log_level", None)
    if val and isinstance(val, str):
        v = val.upper()
        if v in {"DEBUG", "INFO", "WARNING", "WARN", "ERROR", "CRITICAL"}:
            return v
...
def _setup_logging(cfg, sources: dict | None = None) -> None:
