...
"""Serve a persistent, ownership-marked compatibility endpoint for one NIM."""
...
def build_app(config: dict):
    kind = config.get("kind", "embedding")
    if kind == "embedding":
        from .embedding import build_app as build
        return build(config["base_url"], config["model"], config=config)
    if kind == "stt":
        from .stt import build_app as build
        return build(config)
    if kind == "chat":
        from .chat import build_app as build
        return build(config)
    raise ValueError(f"unknown compatibility adapter kind: {kind}")
...
def run() -> None:
    ...
    if any(os.environ.get(key) != value for key, value in markers.items()):
        ...
        os.execvpe(sys.executable, [sys.executable, "-m", "nim_model_adapter", *sys.argv[1:]],
                   os.environ | markers)
    setup_logging("nim-model-adapter")
