...
"""Video animation utilities for mapping visualization."""
...
from inference_SLAM.utilsSLAM import dataPrep
from inference_SLAM.utilsSLAM.utils.vis import vis_result_fast
from inference_SLAM.utilsSLAM.utils.model_utils import compute_clip_features
from inference_SLAM.utilsSLAM.utils.general_utils import to_numpy
from inference_SLAM.utilsSLAM.slam.slam_classes import MapObjectList
from inference_SLAM.utilsSLAM.slam.utils import filter_gobs
...
def main(args):
    images = "/home/rahul/Documents/connor_streaming_code/gxf-xr/apps/video_py_inference/serverInputImages/" + args.input_folder
    objDumps = "/home/rahul/Documents/connor_streaming_code/gxf-xr/apps/video_py_inference/grpcSLAM/grpc_mapping/results/" + args.input_folder + "/object_dumps_for_videos/"
    detection_folder_name = "/home/rahul/Documents/connor_streaming_code/gxf-xr/apps/video_py_inference/grpcSLAM/grpc_segmentation/pkl_files/" + args.input_folder
    
    all_images = glob.glob(images + "/*.jpg")
    all_images = natsort.natsorted(all_images)
    
    meta_path = os.path.join(objDumps, "meta.pkl.gz")
    frame_paths = glob.glob(os.path.join(objDumps, "*.pkl.gz"))
    ...
    video_save_path = Path(args.input_folder) / f"{cfg.gsa_variant}_{cfg.save_suffix}.mp4"

    # Save each video separately in this folder
    sep_video_save_folder = Path(args.input_folder) / "anime_all_frames" / f"{cfg.gsa_variant}_{cfg.save_suffix}"
    ...
    #     dtype=torch.float,
    # )
    cfg.dataset_config = omegaconf.OmegaConf.load(os.path.join("../conceptgraphGRPC" , cfg.dataset_config))
    dataset = dataPrep.IPADDataset(
    ...
    for i in trange(len(frame_paths)):
        color_path = all_images[i]
        image_pil = Image.open(color_path)
        image_rgb = np.array(image_pil)
        
        color_path_temp = Path(color_path)
        detections_path = Path(detection_folder_name) / color_path_temp.name
    ...
    try:
        import imageio
        import imageio_ffmpeg  # required for MP4 backend
    except ImportError as e:
        raise RuntimeError(
            "MP4 animation export requires the optional imageio-ffmpeg backend, "
            "which links to LGPL ffmpeg. Install with `pip install imageio[ffmpeg]` "
            "and accept LGPL terms, or use the PNG-frames-only mode."
        ) from e
