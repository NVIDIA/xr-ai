...
"""Mapping utilities for SLAM operations."""

import torch
import torch.nn.functional as F

from slam.utils.general_utils import Timer
...
def merge_detections_to_objects(
    ...
    for i in range(agg_sim.shape[0]):
        ...
        if agg_sim[i].max() == float('-inf'):
            new_objects.append(nextHistIdx)
            history_map[nextHistIdx] = len(objects)
            detection_list[i]['history_idx'] = nextHistIdx
            nextHistIdx += 1
        ...
        # Merge with most similar existing object
            ...
            if isinstance(j, torch.Tensor):
                j = j.item()
            edited_objects_idx.append(objects[j]['history_idx'])
            
    return objects, edited_objects_idx, new_objects, history_map, nextHistIdx