...
# python scripts/eval_replica_semseg.py --replica_root $REPLICA_ROOT     --replica_semantic_root $REPLICA_SEMANTIC_ROOT     --n_exclude 6     --pred_exp_name dav2_ram_withbg_allclasses_overlap_maskconf0.25_simsum1.2_dbscan.1
import gzip
import os
import glob
...
from slam.datasets.replica_constants import (
...
from slam.utils.vis import get_random_colors
from evaluation.eval import compute_confmatrix, compute_pred_gt_associations, compute_metrics


def get_parser():
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--replica_root", type=Path, default=Path(os.environ.get("REPLICA_ROOT", "")).expanduser()
    )
    parser.add_argument(
        "--replica_semantic_root",
        type=Path,
        default=Path(os.environ.get("REPLICA_SEMANTIC_ROOT", "")).expanduser()
    )
    parser.add_argument(
        "--pred_exp_name", 
        type=str,
    ...
    parser.add_argument(
        "--clip_model_name", type=str, default="mobileclip", choices=["mobileclip", "origclip"],
        help="The name of the CLIP model."
    )
    return parser
...
def eval_replica(
    ...
    if len(result_paths) == 0:
        try:
            if "pkl.gz" in args.pred_exp_name:
                
                result_paths = glob.glob(
                os.path.join(
                    args.replica_root, scene_id, "pcd_saves", "real_time_sys_indi",
                    f"{args.pred_exp_name}"
                )   
                )
            else:
    ...
    assert results['objects'] is not None, f"No objects found in {result_path}"
...
def main(args: argparse.Namespace):
    ...
    if args.clip_model_name == "mobileclip":
        clip_model_name = "MobileCLIP-B"
        clip_pretrained = "datacompdr_lt"
    elif args.clip_model_name == "origclip":
        clip_model_name = "ViT-H-14"
        clip_pretrained = "laion2b_s32b_b79k"
    else:
        raise ValueError(f"Invalid clip_model_name: {args.clip_model_name}")    
    ...
    clip_model, _, clip_preprocess = open_clip.create_model_and_transforms(clip_model_name, clip_pretrained)
